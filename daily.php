...
*/
  ...
  require(DIR_WS_LANGUAGES . $language . '/' . FILENAME_DAILY);

  $breadcrumb->add(NAVBAR_TITLE, tep_href_link(FILENAME_DAILY));

  //Check flash products timer
  $query = "select COUNT(specials_id) as total, expires_date from " . TABLE_SPECIALS . " where expires_date >= '".date('Y-m-d 00:00:00')."' and expires_date <= '".date('Y-m-d 23:59:59')."' and category = '".date('D')."' and status = '1' GROUP BY expires_date order by specials_date_added DESC";
  //var_rh($query);
  $query = tep_db_query($query);
  $infos = tep_db_fetch_array($query);

  //Si pas encore de promo du jour, on en ajoute et on supprime les anciennes
  if (empty($infos['total'])) {
    //Suppression anciennes vente
    $query = "DELETE FROM " . TABLE_SPECIALS . "
        where category IN ('Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun')
        and expires_date < '".date('Y-m-d h:i:s')."'";
    tep_db_query($query);

    //Ajout nouvelles ventes
    //Recuperation des produits ayant une marge superieure a 500%
    $query = "SELECT p.products_id, p.products_price FROM ".TABLE_PRODUCTS." p, ".TABLE_PRODUCTS_DESCRIPTION." pd WHERE pd.language_id = 5 AND p.products_id = pd.products_id AND (pd.Prix_achat * 500 / 100) <= pd.Prix_conseille AND p.products_status = 1 AND p.products_quantity > 0";
    $query = tep_db_query($query);
    $products = array();
    ...
    if (count($products) > 0) {
        ...
        foreach($products as $product) {
            $query = "INSERT INTO " . TABLE_SPECIALS . " SET products_id = ".$product[0].", category = '".date('D')."', specials_date_added = NOW(), expires_date = '".date('Y-m-d 23:59:59')."', specials_new_products_price = ".$product[1];
            tep_db_query($query);
        }
    }
  }
...
<?php new contentBoxHeading_ProdNew($info_box_contents, 'daily_title');?>
...
<?php
  $specials_query_raw = "select
      ...
      p.products_image,
      p.products_price as specials_new_products_price,
      p.item_size,
      pd.Item_size,
      cd.categories_name,
      pd.products_description,
      round(100 - p.products_price * 100 / ((pd.Prix_conseille+18)*1.2)) as discount,
      pd.Prix_conseille
    ...
    where s.expires_date >= '".date('Y-m-d 00:00:00')."'
    and s.category = '".date('D')."'
                    ...
                    <tr>
                        <td style="border:1px solid #FFFFFF; border-width:0 5px 0 0;">
                            <table cellpadding="0" cellspacing="0" border="0">
                                ...
                                <tr>
                                    <td style="height:47px "'.($has_discount ? ' colspan="2"' : '').'><a href="' . $link . '">'.tep_image_button("button_details.gif").'</a><br />'.tep_draw_separator('spacer.gif', '1', '3').'<br /><a href="'.tep_href_link("products_new.php","action=buy_now&products_id=".$p_id).'">'.tep_image_button('button_add_to_cart1.gif').'</a></td>
                                </tr>
                            </table>
                        </td>
                    </tr>
