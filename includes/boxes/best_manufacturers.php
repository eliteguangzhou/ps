...
if (empty($current_category_id)){
	...
	$best_sellers_query = tep_db_query("SELECT m.manufacturers_name,
      m.manufacturers_id,
      SUM(op.products_quantity) as total
    FROM manufacturers m, products p, orders_products op
...
asort($newResult);
  ...
  $str = '<ul>';
    ...
    $str .= '<li class="bg_list"><a href="'.tep_href_link(FILENAME_DEFAULT, 'manufacturers_id='.$id.'&name='.rewrite_name($name)).'">'.$name.'</a>';
  $str .= '</ul>';
  ...
  for ($i = ord("A"), $j = 0; $i <= ord("Z"); $i++, $j++) {
    if($j % 8 == 0)
      $str .= '<br>';
    $str .= '<a class="letters" href="'.tep_href_link('index.php', 'cPath='.$current_category_id.'&letter='.chr($i).'&name='.rewrite_name($cat_name)).'"> &nbsp '.chr($i).'</a> ';
  }
  ...
  while($products = tep_db_fetch_array($products_query))
  	$str .= '<li class="bg_list"> <a  href="'.tep_href_link('index.php', 'cPath='.$current_category_id.'&filter_id='.$products['manufacturers_id'].'&name='.rewrite_name($cat_name).'&name1='.rewrite_name($products['manufacturers_name'])).'">'.$products['manufacturers_name'].'</a>';
  $str .= '</ul>';
/*
