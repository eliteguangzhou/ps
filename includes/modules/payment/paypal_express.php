...
// class methods
    ...
    function before_process() {
      ...
      $params = array('USER' => MODULE_PAYMENT_PAYPAL_EXPRESS_API_USERNAME,
                      ...
                      'LANDINGPAGE' => 'Billing',
                      'LOCALECODE' => $check_server == 'mx' ? 'ES' : strtoupper($check_server),
                      'PAGESTYLE' => $page_style,
                      //'SHIPPINGAMT' => $this->format_raw($order->info['shipping_cost']),
                      'VERSION' => 60);
      ...
      foreach ($order->products as $index => $info) {
         ...
         $p = $this->format_raw($info['final_price']);

	       //Probleme d'arrondi pour les reduc au nombre de produits corrige ici
	       if (stristr($info['name'], 'items bought'))
              $p = round($p);

	       $params['L_NAME'.$index]    = $info['name'];
	       $params['L_AMT'.$index]     = $p;
	       $params['L_NUMBER'.$index]  = $info['model'];
	       $params['L_QTY'.$index]     = $info['qty'];
         $params['ITEMAMT'] += $p * $info['qty'];
         $params['AMT'] += $p * $info['qty'];
         $lasti = $index;
  	  }
      ...
      if ($params['AMT'] > 0.01) {
        ...
        if (($response_array['ACK'] != 'Success') && ($response_array['ACK'] != 'SuccessWithWarning')) {
          tep_redirect(tep_href_link(FILENAME_SHOPPING_CART, 't1='.$params['AMT'].'&error_message=' . stripslashes($response_array['L_LONGMESSAGE0']), 'SSL'));
        }

        return $response_array['TRANSACTIONID'];
      }
