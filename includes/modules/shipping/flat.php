...
// class methods
    ...
    function install() {
      ...
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Co�t de la livraison', 'MODULE_SHIPPING_FLAT_COST', '5.00', 'Le montant forfaitaire de livraison pour toute commande utilisant cette m�thode.', '6', '0', now())");
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) values ('TVA applicable', 'MODULE_SHIPPING_FLAT_TAX_CLASS', '0', 'S�lectionnez la TVA applicable sur le montant de la livraison.', '6', '0', 'tep_get_tax_class_title', 'tep_cfg_pull_down_tax_classes(', now())");
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) values ('Zone de livraison', 'MODULE_SHIPPING_FLAT_ZONE', '0', 'Si une zone est s�lectionn�e, elle sera la seule � proposer ce mode de livraison.', '6', '0', 'tep_get_zone_class_title', 'tep_cfg_pull_down_zone_classes(', now())");
	...
	function get_offers() {
	  ...
	  for ($i = 1; $i <= $max; $i ++) {
		  $str .= '<tr class="'.($i%2?'second':'').'">
			...
			<td>'.(($promo = $this->get_promo($i)) > 0 ? $currencies->currencies[$currency]['symbol_left'].round($promo * $currencies->currencies[$currency]['value']).$currencies->currencies[$currency]['symbol_right'] : 0).'</td>
		  </tr>';
	  }
	  return $str;
	}
