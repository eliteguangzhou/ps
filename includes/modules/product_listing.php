  ...
  if ($listing_split->number_of_rows > 0) {
    ...
    while ($listing = tep_db_fetch_array($listing_query)) {
      $rows++;
        ...
        //$product_query = tep_db_query("select p2c.categories_id, p.products_quantity, pd.products_description, pd.products_id, pd.Brand, pd.Gender, pd.Gamme, pd.Prix_achat, pd.Note, pd.Annee, pd.Item_size, pd.Prix_conseille, pd.Type from " . TABLE_PRODUCTS . " as p, " . TABLE_PRODUCTS_DESCRIPTION . " as pd, " . TABLE_PRODUCTS_TO_CATEGORIES . " p2c where p.products_id = pd.products_id and p.products_id = p2c.products_id AND pd.products_id = '" . (int)$listing['products_id'] . "' and pd.language_id = '" . (int)$languages_id . "' ".$range_where);
        $product = $listing;//tep_db_fetch_array($product_query);
        $listing['products_price'] = get_price($listing['products_price']);

        $link = tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $listing['products_id'].rewrite_product($listing));
      ...
      for ($col=0, $n=sizeof($column_list); $col<$n; $col++) {
        ...
        switch ($column_list[$col]) {
          ...
          case 'PRODUCT_LIST_NAME':
            $lc_align = '';
            $p_name = $lc_text = '<a href="' . $link . '">' . display_product_name($listing['products_name'], $product) . '</a>';
          ...
          case 'PRODUCT_LIST_IMAGE':
            $lc_align = 'center';
            $p_pic = '<a href="' . $link . '">' . tep_image(DIR_WS_PWS_IMAGE . $listing['products_image'], $listing['products_name'], SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT) . '</a>';
            break;
                    ...
                    <tr>
                        <td colspan="2" style="border:1px solid #FFFFFF; border-width:0 15px 0 0;">
                            <table cellpadding="0" cellspacing="0" border="0">
                                ...
                                '.(
                                get_adviced_price($product['Prix_conseille']) > $listing['products_price']
                                ? '
                                <tr>
                                    <td class="vam instead_of">'.TEXT_INSTEAD_OF.' '.$currencies->display_price(get_adviced_price($product['Prix_conseille']), tep_get_tax_rate($listing['products_tax_class_id'])).'</td>
                                </tr>'
                                : '').
                                (
                                get_adviced_price($product['Prix_conseille']) > $listing['products_price']
                                ? '
                                <tr>
                                    <td class="vam instead_of">'.TEXT_SAVING.' '.$currencies->display_price(get_adviced_price($product['Prix_conseille']) - $listing['products_price']).' ('. (100 - floor($listing['products_price'] / get_adviced_price($product['Prix_conseille']) * 100)) .'%)</td>
                                </tr>'
                                : '').
                                (
                                ...
                                <tr>
                                    <td style="height:47px "><a href="' . $link . '">'.tep_image_button("button_details.gif").'</a><br />'.tep_draw_separator('spacer.gif', '1', '3').'<br /><a href="'.tep_href_link("products_new.php","action=buy_now&products_id=".$p_id).'">'.tep_image_button('button_add_to_cart1.gif').'</a></td>
                                </tr>
                            </table>
                        </td>
                    </tr>
...
if(empty($_GET['manufacturers_id']) && empty($_GET['keywords']) && isset($product['categories_id']))
