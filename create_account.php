...
// needs to be included earlier to set the success message in the messageStack
  ...
  $form_query = '';
  if (!tep_session_is_registered('is_neta') || !$is_neta && isset($_GET['neta'])) {
    $is_neta = ACTIVATE_DISCOUNT_NETA && isset($_GET['neta']);
    tep_session_register('is_neta');
  }
  ...
  if (isset($HTTP_POST_VARS['action']) && ($HTTP_POST_VARS['action'] == 'process')) {
    ...
    if (strlen($postcode) == 5 && in_array(substr($postcode,0, 2), $blocked_postcode)) {
      $error = true;
...
$email_text=str_replace('vnom', $firstname, $email_text);
      ...
      if (ACTIVATE_DISCOUNT_NETA && $is_neta) {
        $date = getdate(strtotime("+ 3 month"));
        $in_3_months = $date['year'].'-'.lt_10($date['mon']).'-'.lt_10($date['mday']).' '.lt_10($date['hours']).':'.lt_10($date['minutes']).':'.lt_10($date['seconds']);
        $i = 0;
        $code = array();
        while($i++ < 3) {
          $code[$i] = gen_coupon_code(SPONSORSHIP_CODE_LENGTH);
          $sponsorship->generate_discount($code[$i], DISCOUNT_NETA, $email_address, $in_3_months, 0, 2);
        }
        tep_mail($name, $email_address, EMAIL_SUBJECT_NETA, sprintf(EMAIL_TEXT_NETA, join('<br />'."\n", $code)), STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS);
        tep_session_register('code');
      }
...
<?php
  if(ACTIVATE_DISCOUNT_NETA && $is_neta)
    echo tep_image(DIR_WS_IMG.'neta.jpg', 'Promo', '608', '700', ' align="center" ');
  tep_draw_heading_top(!ACTIVATE_DISCOUNT_NETA || !$is_neta);
?>
...
?>
              ...
              <tr>
                ...
                <td class="main"><?php echo tep_draw_input_field('email_address_confirm', $is_sponsorship ? $_GET['email'] : '') . '&nbsp;' . (tep_not_null(ENTRY_EMAIL_ADDRESS_CONFIRM_TEXT) ? '<span class="inputRequirement">' . ENTRY_EMAIL_ADDRESS_CONFIRM_TEXT . '</span>': ''); ?></td>
              </tr>
            </table>
