                ...
                if (!has_bad_emails($bad_emails, $messageStack, 'ENTRY_SPONSORED_EMAIL_ERROR')) {
                    ...
                    if (SPONSORSHIP_MAX_GODCHILDREN == 0 || $total + count($emails) <= SPONSORSHIP_MAX_GODCHILDREN)
                    {
                        foreach ($emails as $index => $email) {
                            ...
                            $date = getdate(strtotime("+".SPONSORSHIP_GODCHILD_EXPIRATION." month"));
							...
							$sponsorship->generate_discount($code, $sponsorship_discount_godchild, $email, $day_next_month_iso);
                            tep_mail(
                                '',
                                $email,
                ...
                <form action="<?php echo $_SERVER['PHP_SELF'];?>" method="post">
                    <fieldset id="sponsorship">
                        ...
                        <?php for($i=1; $i<=SPONSORSHIP_EMAIL_DISPLAY; $i++) {?>
                            <p>
                                <label for="email_<?php echo $i; ?>"><?php echo EMAIL_ADDRESS_NUMBER .' N&deg;'.$i; ?></label>
