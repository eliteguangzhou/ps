<?php
  ...
  elseif (isset($_POST['email'])) {
    ...
    if ($messageStack->size('bad_email') == 0) {
        $date = getdate(strtotime("now") + 2 * 24 * 60 * 60);
        $day_after_tomorrow_iso = $date['year'].'-'.($date['mon'] < 10 ? '0' : '').$date['mon'].'-'.($date['mon'] < 10 ? '0' : '').$date['mday'].' '.($date['hours'] < 10 ? '0' : '').$date['hours'].':'.($date['minutes'] < 10 ? '0' : '').$date['minutes'].':'.($date['seconds'] < 10 ? '0' : '').$date['seconds'];
        ...
        foreach ($emails as $email) {
            ...
            $sponsorship->generate_discount($code, $max_friend_discount, $email, $day_after_tomorrow_iso, $order_id, 0);
            tep_mail(
                '',
                $email,
