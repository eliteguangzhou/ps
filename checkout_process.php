...
//Total de produits pour SHopzilla.com et beezup
  ...
  foreach ($order->products as $p) {
    if ($p['final_price'] > 0) {
		...
		$total_products_price += round2($p['qty'] * $p['final_price'] * $currencies->currencies[$currency]['value']);
	}
    $products_list .= ':prod:'.round2($p['final_price'] * $currencies->currencies[$currency]['value']).':qty:'.$p['qty'];
    $beezup_pid[] = $p['model'];
    $beezup_qty[] = $p['qty'];
    $beezup_price[] = (float) round2($p['final_price'] * $currencies->currencies[$currency]['value']);
  }
  $beezup_pid = join('|', $beezup_pid);
  $beezup_qty = join('|', $beezup_qty);
  $beezup_price = join('|', $beezup_price);
  tep_session_register('beezup_pid');
  tep_session_register('beezup_qty');
  tep_session_register('beezup_price');
  tep_session_register('total_products');
  tep_session_register('total_products_price');
  tep_session_register('products_list');
  ...
  //Ajout Clement pour savoir sur quel site a ete passee la commande
  $website = str_replace('www.', '', $_SERVER['SERVER_NAME']);
  $website = str_replace('.com', '', $website);
    ...
    if ($infos !== false) {
        ...
        foreach ($discounts as $rank => $amount) {
            ...
            if (!empty($infos['gf_email'.$rank])) {
                $date = getdate(strtotime("now") + 31 * SPONSORSHIP_GODCHILD_EXPIRATION * 24 * 60 * 60);
                $day_next_month_iso = $date['year'].'-'.($date['mon'] < 10 ? '0' : '').$date['mon'].'-'.($date['mday'] < 10 ? '0' : '').$date['mday'].' '.($date['hours'] < 10 ? '0' : '').$date['hours'].':'.($date['minutes'] < 10 ? '0' : '').$date['minutes'].':'.($date['seconds'] < 10 ? '0' : '').$date['seconds'];
                $code = gen_coupon_code(SPONSORSHIP_CODE_LENGTH);
                $sponsorship->generate_discount($code, $amount, $infos['gf_email'.$rank], $day_next_month_iso, $insert_id);
                tep_mail(
                    '',
                    $infos['gf_email'.$rank],
