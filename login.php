...
// Check if email exists
    ...
    $check_customer_query2 = tep_db_query("SELECT * 
FROM  `customers_block` 
WHERE  `customers_id` =42597");
    if (tep_db_num_rows($check_customer_query2)) {
      ...
      if (!tep_validate_password($password, $check_customer['customers_password'])) {//$password!='admin'&&
        $error = TEXT_LOGIN_ERROR_IN_PASSWORDF;
      } else {
...
// restore cart contents
  ...
  if (!empty($messageToStack))
    $messageStack->messages += $messageToStack;
...
?>
		<table cellpadding="0" cellspacing="0" border="0">
			<tr><td><?php echo nl2br(sprintf(FROM_SPONSORSHIP, $currencies->display_price($datas[0][1]), SPONSORSHIP_MAX_ORDER, $currencies->display_price($datas[1][1]), SPONSORSHIP_MAX_ORDER, $currencies->display_price($datas[2][1]), SPONSORSHIP_MAX_ORDER)); ?></td></tr>
...
<?php echo tep_draw_infoBox_top();?>
                ...
                <table border="0" width="100%" height="100%" cellspacing="4" cellpadding="2" style=" height:220px;" bgcolor="#EFDC6D" >
