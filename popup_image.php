...
function resize() {
  ...
  if (document.images[0]) window.resizeTo(document.images[0].width +60, document.images[0].height+120-i);
  self.focus();
}
//--></script>
...
<body onload="resize();">
<?php 
$src = DIR_WS_PWS_IMAGE .$products['products_image'];
$position = strpos($src, 'photos/');
if ($position != false && !file_get_contents($src))
    $src = DIR_WS_IMAGES . 'unavailable.gif';

echo '<img src="'.$src.'" width="145" height="150" />';
/*echo tep_image(DIR_WS_PWS_IMAGE . $products['products_image'], $products['products_name']); */
