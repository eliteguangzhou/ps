...
//Ajout anti hack
  preg_match_all('/\//', $PHP_SELF, $rs);
  if (count($rs[0]) > 2) {
      $PHP_SELF = substr($PHP_SELF, strpos($PHP_SELF, '/', 2) + 1);
...
if (MAINTENANCE_ENABLE) {
	...
	if (!isset($_SESSION['ghj']))
		die('Le site est actuellement en maintenance et sera de nouveau disponible dans quelques minutes.<br />Veuillez nous excuser pour la gêne occasionnée.');
}
?>
