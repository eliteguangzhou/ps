...
*/
  ...
  require(DIR_WS_LANGUAGES . $language . '/' . FILENAME_MONTHLY);

  $breadcrumb->add(NAVBAR_TITLE, tep_href_link(FILENAME_MONTHLY));
?>
...
<?php
  ...
  DISTINCT cd.categories_name, p.products_quantity, p.products_image, pd.products_name, pd.products_description, pd.Gamme, pd.Prix_conseille, pd.Type, pd.Gender, p.products_id, p.manufacturers_id, p.products_price, p.products_tax_class_id, IF(s.status, s.specials_new_products_price, NULL) as specials_new_products_price, IF(s.status, s.specials_new_products_price, p.products_price) as final_price from " . TABLE_SELECTIONS . " se, " . TABLE_PRODUCTS_DESCRIPTION . " pd, " . TABLE_PRODUCTS . " p left join " . TABLE_MANUFACTURERS . " m on p.manufacturers_id = m.manufacturers_id left join " . TABLE_SPECIALS . " s on p.products_id = s.products_id, " . TABLE_PRODUCTS_TO_CATEGORIES . " p2c, " . TABLE_CATEGORIES_DESCRIPTION . " cd  where p2c.categories_id = cd.categories_id and cd.language_id = ".(int)$languages_id." AND p.products_status = '1' and p.products_quantity > 2 and p.products_id = p2c.products_id and pd.products_id = p2c.products_id and pd.language_id = '" . (int)$languages_id . "' and p.products_model = se.products_model and se.type= 'prix_en_folie'";
